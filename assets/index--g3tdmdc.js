...
✅ Test completed. Quantities decreased as prizes were won (except unlimited).`),console.log("💡 Reset the game to restore original quantities.")}logSpin(e){const t={prizeName:e.name,gameMode:"Probability"};storageManager.addLog(t)}}document.addEventListener("DOMContentLoaded",()=>{window.slotMachine=new j,window.testProbability=(f=1e3)=>{window.slotMachine.testProbabilityAccuracy(f)},console.log("🎰 Slot Machine loaded! Test probability with: testProbability(1000)")});class L{constructor(){this.isExpanded=!1,this.button=null,this.icon=null}initialize(){var e;this.button=document.getElementById("expandButton"),this.icon=(e=this.button)==null?void 0:e.querySelector(".expand-icon"),this.button&&this.button.addEventListener("click",()=>this.toggleExpand())}toggleExpand(){this.isExpanded=!this.isExpanded,this.isExpanded?(document.body.classList.add("expanded-mode"),this.button.title="Collapse to normal size",this.icon.textContent="⛶"):(document.body.classList.remove("expanded-mode"),this.button.title="Expand to full screen",this.icon.textContent="⛶")}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{window.expandButtonManager=new L,window.expandButtonManager.initialize()}):(window.expandButtonManager=new L,window.expandButtonManager.initialize());class W{constructor(){this.mode=this.getStoredMode()||null,this.listeners=[],this.initialized=!1,this.configs={"high-quality":{name:"High Quality Mode",description:`Full visual effects and smooth animations
